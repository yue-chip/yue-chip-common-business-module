import {
  message_default
} from "./chunk-DUF3E4CA.js";
import "./chunk-OST7MHDL.js";
...
// node_modules/@yue-chip/yue-chip-frontend-core/axios/axios.ts
var pending = [];
...
service.interceptors.request.use((config) => {
  ...
  if (config.url.indexOf("http") > -1) {
    config.url = config.url + (config.url.indexOf("?") > -1 ? "&" : "?") + "_t=" + (/* @__PURE__ */ new Date()).getTime();
  } else {
    config.url = import.meta.env.VITE_BASEURL + import.meta.env.VITE_BASEAPI + config.url + (config.url.indexOf("?") > -1 ? "&" : "?") + "_t=" + (/* @__PURE__ */ new Date()).getTime();
  }
  if (!config.neverCancel) {
    config.cancelToken = new CancelToken3((c) => {
...
service.interceptors.response.use(
  (response) => {
    ...
    if (response.status != 200) {
      message_default.error(response.status);
    }
    if (response.data.status === 403) {
      message_default.error("登陆异常，请重新登陆！");
      setTimeout(function() {
        if (self != top) {
      ...
      if (response.data.message) {
        message_default.error(response.data.message);
      } else if (response.data.exceptionMessage) {
        message_default.error(response.data.exceptionMessage);
      }
    }
  ...
  (error) => {
    if (error & error.message) {
      message_default.error(error.message);
    }
    pending = [];
...
var axiosGet = function(url, params, success, fail, finallyF) {
...
var axiosPut = function(url, params, success, fail, finallyF) {
...
var axiosPost = function(url, params, success, fail, finallyF) {
...
var axiosDelete = function(url, params, success, fail, finallyF) {
...
var axios_default2 = { service, axiosGet, axiosPut, axiosPost, axiosDelete };
export {
  axios_default2 as default
};
//# sourceMappingURL=@yue-chip_yue-chip-frontend-core_axios_axios.js.map
