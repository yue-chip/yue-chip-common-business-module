<template>
    <div>
        ...
        <a-card>
            <a-table rowKey="id" :columns="columns" :data-source="dataList" :loading="loading" :pagination="false"
                :scroll="{ y: 500 }">
                <template #bodyCell="{ column, text, record }">
                    <template v-if="column.key === 'operation'">
                        <a-space :size="5">
                            ...
                            <!--                添加目录-->
                            <!--              </a-button>-->
                            <a-button v-if="record.type.name === 'CATALOG'" size="small"
                                @click="add(null, record.id, searchModel.scope, 'MENU')">
                                <template #icon>
                                    <PlusOutlined />
                                </template>
                                添加菜单
                            </a-button>
                            <a-button v-if="record.type.name === 'MENU'" size="small"
                                @click="add(null, record.id, searchModel.scope, 'FUNCTION')">
                                <template #icon>
                                    <PlusOutlined />
                                </template>
                                添加功能
        ...
        <a-modal width="800px" v-model:visible="visible" title="添加/修改资源" cancelText="取消" okText="保存"
                ...
                <a-row>
                    ...
                    <a-col :span="12">
                        <a-form-item label="英文名称">
                            <a-input placeholder="请输入英文名称" v-model:value="addOrUpdateModel.nameEn" />
                        </a-form-item>
                    </a-col>
                    <a-col :span="12">
                ...
                <a-row>
                    <a-col :span="12">
                        <a-form-item label="URL" name="url" ref="url">
                            <a-input
                                v-bind:disabled="addOrUpdateModel.type === 'FUNCTION' || addOrUpdateModel.type === 'CATALOG'"
                                placeholder="请输入URL" v-model:value="addOrUpdateModel.url" />
                        </a-form-item>
                    </a-col>
                    <a-col :span="12">
