<template>
    <div>
        ...
        <a-card>
            <a-table rowKey="id" :columns="columns" :data-source="dataList" :pagination="pagination" :loading="loading"
                :scroll="{ y: 440 }">
                <template #bodyCell="{ column, text, record }">
                    <template v-if="column.key === 'operation'">
                        <a-space :size="5">
                            ...
                            <a-button v-if="record.isDefault === false" size="small" type="dashed"
                                @click="del(record.id)">
                                <template #icon>
                                    <DeleteOutlined />
                                </template>
        ...
        <a-modal width="800px" v-model:visible="visible" title="添加/修改角色" cancelText="取消" okText="保存"
                ...
                <a-row>
                    <a-col :span="24">
                        <a-form-item :labelCol="{ span: 1.9, offset: 0 }" label="备注" name="remark" ref="remark">
                            <a-textarea placeholder="请输入备注" :auto-size="{ minRows: 5, maxRows: 5 }"
                                v-model:value="addOrUpdateModel.remark" />
                        </a-form-item>
                    </a-col>
                </a-row>
            </a-form>
        ...
        <a-modal width="400px" height="600px" v-model:visible="permissionsVisible" title="权限" cancelText="取消"
            okText="保存" :destroyOnClose=true :mask=true :maskClosable=false @cancel="permissionsCancel"
            @ok="permissionsSave">
            <a-tree v-model:checkedKeys="permissionsCheckedKeys" default-expand-all checkable :height="600"
                :tree-data="treeData">
            </a-tree>
        </a-modal>

        <a-modal width="400px" height="600px" v-model:visible="userVisible" title="人员" cancelText="取消" okText="保存"
            :destroyOnClose=true :mask=true :maskClosable=false @cancel="userCancel" @ok="userSave">
            <a-select v-model:value="roleUser" mode="tags" style="width: 100%" placeholder="请选择人员"
                :options="userOptions"></a-select>
        </a-modal>
    </div>
</template>
