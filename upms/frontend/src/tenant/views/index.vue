<template>
    <div>
                    ...
                    <a-col :span="6">
                        <a-form-item label="状态" name="state" ref="state">
                            <a-select v-model:value="searchModel.state" allowClear>
                                <a-select-option v-for="(state, index) in stateList"
                                    :value="state.key">{{ state.desc }}</a-select-option>
                            </a-select>
                        </a-form-item>
                    </a-col>
                    <a-col :span="6">
            ...
            <a-table rowKey="id" :row-selection="rowSelection" :columns="columns" :data-source="dataList"
                ...
                <template #bodyCell="{ column, text, record }">
                    <template v-if="column.key === 'state'">
                        <a-switch @change="stateChange(record.id, record.stateTmp)" v-model:checked="record.stateTmp"
        ...
        <a-modal width="800px" v-model:visible="visible" title="添加/修改租户" cancelText="取消" okText="保存"
                ...
                <a-row>
                    ...
                    <a-col :span="24">
                        <a-form-item label="访问域名" name="requestDomain" ref="requestDomain">
                            <a-input placeholder="请输入访问域名(www.baidu.com)多个域逗号隔开"
                                v-model:value="addOrUpdateModel.requestDomain" />
                        </a-form-item>
                    </a-col>
                </a-row>
            </a-form>
