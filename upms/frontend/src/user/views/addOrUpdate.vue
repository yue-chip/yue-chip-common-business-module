  ...
  <a-card class="card" :bordered="false">
    <a-form ref="from" :rules="rules" :model="addOrUpdateModel" layout="horizontal" :label-col="{ span: 4, offset: 0 }"
      ...
      <a-form-item label="所属机构" name="organizationalId" ref="organizationalId">
        <a-tree-select v-model:value="addOrUpdateModel.organizationalId" tree-data-simple-mode style="width: 100%"
          :tree-data="treeData" allow-clear :show-checked-strategy="SHOW_PARENT" placeholder="请选择所属机构"
      ...
      <a-form-item label="头像">
        <a-upload v-model:file-list="fileList" name="avatar" list-type="picture-card" class="avatar-uploader"
          :show-upload-list="false" action="/api/common/file/upload" :before-upload="beforeUpload" :headers="headers"
          @change="handleChange">
      ...
      <a-form-item label="其它照片">
        <a-upload v-model:file-list="otherPhotoUrlList" name="avatar" list-type="picture-card" class="avatar-uploader"
          :show-upload-list="false" action="/api/common/file/upload" :before-upload="beforeUpload" :headers="headers"
