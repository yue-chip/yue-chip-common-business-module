 ...
 */
@RestController("userResourcesController")
...
public class ResourcesController extends BaseControllerImpl implements BaseController {
    ...
    public IResultData<List<ResourcesTree>> resourcesTree(@RequestParam(value = "作用域", defaultValue = "CONSOLE") Scope scope) {
    ...
    public IResultData<Boolean> checkCodeIsExist(@NotBlank(message = "编码不能为空") @Parameter(description = "编码") @RequestParam(name="code",required = true)String code,
                                                 @Parameter(description = "修改需要传id，新增则不需要传") @RequestParam(name="id",required = false) Long id){
    ...
    public IResultData<Boolean> checkNameIsExist(@NotBlank(message = "名称不能为空") @Parameter(description = "名称") @RequestParam(name="name",required = true)String name,
                                                 @Parameter(description = "修改需要传id，新增则不需要传") @RequestParam(name="id",required = false)Long id,
                                                 @NotNull(message = "父节点id不能为空") @Parameter(description = "父节点id") @RequestParam(name="parentId",required = true)Long parentId){
        ResultData resultData = ResultData.builder().data(false).build();
    ...
    public IResultData<Boolean> checkUrlIsExist(@NotBlank(message = "url不能为空") @Parameter(description = "url") @RequestParam(name="url",required = true)String url,
                                                @Parameter(description = "修改需要传id，新增则不需要传") @RequestParam(name="id",required = false) Long id){
        ResultData resultData = ResultData.builder().data(false).build();
        Optional<Resources> optional = resourcesRepository.findByUrl(url);
        if (optional.isPresent()){
            Resources resources = optional.get();
            resultData.setData(resources.checkCodeIsExist(resources,url,id));
        }
        return resultData;
    }
    ...
    @PutMapping("/update")
    public IResultData update(@RequestBody @Validated({Validator.Insert.class}) ResourcesUpdateDto resources){
        resources.setCode(resources.getCode().trim().toUpperCase());
        return ResultData.builder().build();
    }
