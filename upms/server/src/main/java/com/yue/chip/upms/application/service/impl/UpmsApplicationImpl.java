...
//    @GlobalTransactional(rollbackFor = {Exception.class})
    ...
    public void saveAppUser(UserAddOrUpdateDto userAddOrUpdateDto) {
        //检查用户是否存在
        User user = User.builder().username(userAddOrUpdateDto.getUsername()).build();
        if (Objects.isNull(userAddOrUpdateDto.getId())) {
            Assert.isFalse(user.checkUsernameIsExist(), () -> {
                return new BusinessException("该帐号已存在");
            });
        }
        //保存app用户
...
//        upmsRepository.updateUserPassword(CurrentUserUtil.getCurrentUserId(),passwordEncoder.encode(SecureUtil.md5(userUpdatePasswordDto.getPassword())));
        upmsRepository.updateUserPassword(
                Objects.isNull(userUpdatePasswordDto.getUserId()) ? CurrentUserUtil.getCurrentUserId() : userUpdatePasswordDto.getUserId(),
